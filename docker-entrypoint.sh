...
if [ "$WORDPRESS_ENV" == "local" ]; then
    echo >&2 "Iptables port map"
    echo >&2 "$WORDPRESS_HOST_PORT"
    iptables -t nat -A OUTPUT -o lo -p tcp --dport "$WORDPRESS_HOST_PORT" -j REDIRECT --to-port 80
fi

echo >&2 "apache2-foreground"
exec "$@"
